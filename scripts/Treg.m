...
for iyr = syr:eyr  
    ...
    fin     = sprintf('~/work/DATA/ERAint/2m/erain.2mT.daily.%d.nc',iyr);
    finp    = sprintf('~/work/DATA/ERAint/2m/erain.2mT.daily.%d.nc',iyr-1);
    disp(['Data read from ' fin])
    ncid      = netcdf.open ( fin,'NC_NOWRITE' );
    ncidp      = netcdf.open ( finp,'NC_NOWRITE' );

    varid     = 3;
    [name,type,dimids,natts] = netcdf.inqVar(ncid,varid);
    if(name=='t2m'); 
        t2m     = netcdf.getVar (ncid,varid,'short');
        scale = netcdf.getAtt(ncid,varid,'scale_factor');
        offset= netcdf.getAtt(ncid,varid,'add_offset');
        tc = single(t2m)*scale+offset;   
        clear t2m
         display(['size of var (',name,') = ',sprintf(' %d',size(tc))]);

        t2m    = netcdf.getVar (ncidp,varid,'short');
        scale = netcdf.getAtt(ncid,varid,'scale_factor');
        offset= netcdf.getAtt(ncid,varid,'add_offset');
        tp= single(t2m)*scale+offset;   
    ...
    [name,type,dimids,natts] = netcdf.inqVar(ncid,varid);
    if(name == 'time'); 
        timec     = netcdf.getVar (ncid,varid);
         display(['size of time = ',sprintf(' %d',size(timec))]);
        timep    = netcdf.getVar (ncidp,varid);
        time0 = [timep' timec'];
        time = time0';
        clear timec timep time0
    else
        disp('Check time');
        exit
    end;
