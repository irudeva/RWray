...
%     netcdf.close(ncid);
   ...
   for it =1:1     %size(time,1)
       disp(['Time: ' num2str(it) ]) ;
       for il = 1:size(lev)
%         disp(['Level: ' num2str(lev(il)) ]) ;
        ussn = zUbar(:,:,il,it1:it2);
        vssn = zVbar(:,:,il,it1:it2);


        % Climatology
        u0=(squeeze(mean(ussn(:,:,:,:),4)))';
        % u0=(squeeze(ussn))';
        [m,n]=size(u0);
        v0=(squeeze(mean(vssn(:,:,:,:),4)))';
    ...
    fout = sprintf('../../output/Ks/Ks_map/Ks.erain.%s.%d.nc',ssn,iyr)
...
%     end if
    ...
    % write data
    netcdf.putVar(ncoutid,varid_t,0,length(time(1)),time(1));
